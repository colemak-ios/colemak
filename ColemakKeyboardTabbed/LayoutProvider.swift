...
class CustomDeviceBasedLayoutService: KeyboardLayout.DeviceBasedLayoutService {
    ...
    static var colemak: InputSet {
        ...
        var firstRow = "qwfpgjluy".map({ InputSet.Item(String($0)) })
        
        var secondRow = "arstdhneio".map({ InputSet.Item(String($0)) })
...
class CustomiPhoneLayoutService: KeyboardLayout.iPhoneLayoutService {
    override func topTrailingActions(for actions: KeyboardAction.Rows, context: KeyboardContext) -> KeyboardAction.Row {
